...
Nothing in `web/` was modified. The build was run with a relative base so the asset paths in the
copied `index.html` are `./assets/...` and work under `samad-zeeshan.github.io/Tally/`:
...
| Amara 375.00, Ben 125.00, world -500.00 (the zero line figure) | `docs/data/tally-run.json`, step `transfer` | The `book` array of that step. Bar heights are fractions of one unit where 1 is 500.00, so the two halves are equal by construction and measure equal in the browser (66 + 22 = 88 px at 360 px wide). |
...
Note for whoever edits `docs/correctness.md` next: line 63 of that file carries the same inverted
phrasing, "N = one thread per two cores (clamped to 8 to 32)". It was left alone because that file
is linked from a resume that has already been submitted, and changing a linked document after the
fact is worse than the error. It should be corrected the next time that file is touched for another
reason.
...
- Page weight: one 21 KB HTML file plus the app's 219 KB of JavaScript, 17 KB of CSS and the 13 KB
  stand-in.
- Nothing under `web/src`, no Java source, `web/vite.config.ts` and `web/package.json` are unchanged.
  `web/dist/` is regenerated but is listed in `web/.gitignore`.

## Known rough edges

- The app's own copy inside the frame still uses its engineering vocabulary ("double entry",
  "idempotent transfers", "reconciliation"), because `web/src` may not be edited. The section
  immediately below the frame, "What you just watched", translates each of those three claims into
  plain words for exactly this reason.
